...
- **Dynamic Cover Evaluation**: AI continuously evaluates available cover in real-time rather than relying on pre-placed waypoints.
- **Destructible Environment**: AI reacts to environmental changes (e.g., destroyed cover, blocked paths).
- **Weather and Time Effects**: Adjust behavior based on weather (reduced visibility in rain) and time of day (night patrols).
- **Interactive Objects**: AI can use doors, vehicles, or tools in the environment.

**Implementation Approach**:
- Implement raycasting-based cover detection system.
- Add environmental sensors that update periodically.
- Integrate with gameData for weather/time state.
